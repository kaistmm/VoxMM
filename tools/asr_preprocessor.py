...
def worker(buff):
    ...
    for seg in metadata['segments']:
        if args.use_vid_index:
            wav_out, video_path_out, txt_path = cu.output_destination(str(fidx), seg, output_dir, args.dataset_style)
        else:
            wav_out, video_path_out, txt_path = cu.output_destination(fn, seg, output_dir, args.dataset_style)

        try:
            if not args.no_wav:
                os.makedirs(os.path.dirname(wav_out), exist_ok=True)
                wu.crop_wav(src_wav, seg['start'], seg['end'], sr, args.volume, wav_out)

            if not args.no_video:
                os.makedirs(os.path.dirname(video_path_out), exist_ok=True)
                if len(seg['face_track']) == 1:
                    video_batch.append((
                        seg['face_track'][0]['index'],
                        seg['start'],
                        seg['end'],
                        video_path_out,
                        args.volume,
            ...
            if not args.no_script:
                ...
                if args.dataset_style == 'librispeech':
                    with open(txt_path, 'a') as f:
                        f.write('{} {}\n'.format(os.path.basename(wav_out).replace('flac', ''), script_gen(seg['text'])))
                elif args.dataset_style == 'lrs3':
                    with open(txt_path, 'w') as f:
                        f.write('Text:  {}\n'.format(script_gen(seg['text'])))
                else:
                    raise Exception(f"Invalid dataset style input: {args.dataset_style}")

        except Exception as e:
            print('Error occur while processing the segment. Skip this segment. file: {}, seg idx: {}'.format(fn, seg['segment_index']))
            print(f'{str(e)}')

    if not args.no_video and len(video_batch) > 0:
        errors = vid_gen.process_sorted(video_batch)
        for err_path, err_msg in errors:
            print(f'Error in video processing: {err_path} - {err_msg}')
